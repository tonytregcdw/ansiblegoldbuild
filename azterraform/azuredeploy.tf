...
resource "azurerm_virtual_network_peering" "peer1" {
  ...
  resource_group_name       = azurerm_resource_group.rg1.name
  virtual_network_name      = azurerm_virtual_network.region1-vnet1-hub1.name
  remote_virtual_network_id = azurerm_virtual_network.TT_Infrastructure_RG-vnet.id
...
resource "azurerm_network_interface" "r1-ws201901-nic" {
  ...
  ip_configuration {
    name                          = "r1-ws201901-ipconfig"
    subnet_id                     = azurerm_subnet.region1-vnet1-snet1.id
    private_ip_address_allocation = "Dynamic"
	  public_ip_address_id = azurerm_public_ip.r1-ws201901-pip.id
  }
...
resource "azurerm_windows_virtual_machine" "r1-ws201901-vm" {
  name                = "r1-ws201901-vm"
  depends_on = [ azurerm_key_vault.kv1 ]
  resource_group_name = azurerm_resource_group.rg1.name
  location            = var.loc1
  size                = var.vmsize-ws2019gold
  admin_username      = var.adminusername
  admin_password      = azurerm_key_vault_secret.vmpassword.value
  network_interface_ids = [
    azurerm_network_interface.r1-ws201901-nic.id,
  ]
...
#Run ansible setup script on ws201901-vm
